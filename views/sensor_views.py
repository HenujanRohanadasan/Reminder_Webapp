...
def temperature_live():
    return render_template('sensor_data.html', user=current_user, href='temp', chart_type='line', active='temperature', live=True)


@sensor_views.route('/sensor_data/live/humidity', methods=['GET'])
@login_required
def humidity_live():
    return render_template('sensor_data.html', user=current_user, href='hum', chart_type='bar', active='humidity', live=True)
...
@login_required
def temperature():
    return render_template('sensor_data.html', user=current_user, href='temp', chart_type='line', active='temperature', live=False)


@sensor_views.route('/sensor_data/humidity', methods=['GET'])
@login_required
def humidity():
    return render_template('sensor_data.html', user=current_user, href='hum', chart_type='bar', active='humidity', live=False)
